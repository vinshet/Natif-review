...
def write_to_dataset(file_info):
    ...
    if check_response(upload_details):
        ...
        if upload_details.json()["processing_status"] == "pending":
            ...
            doc_dataset=create_dataframe(ocr_response,file_name)
...
for file_path in input_folder_paths:
    ...
    if file_type not in allowed_filetypes:
        logger.error("File type does not match for document {}".format(os.path.basename(file_path)))
        continue
    fil_upload_details = upload_files(file_path,'other',headers,input_handle,endpoint)
    ...
    if resp_chk == "quota_exceeded":
        ...
        write_to_dataset(file_info)
ocr_dataset={'Document_name':file_name,'text':text,'page no':page_count,'box_id':box_id,'x1':x1_pos,'y1':y1_pos,'x2':x2_pos,'y2':y2_pos,'entropy':entropy,'page_width':width,'page_height':height}
ocr_dataframe=pd.DataFrame(ocr_dataset,columns=['Document_name','text','entropy','page no','page_width','page_height','box_id','x1','y1','x2','y2'])
