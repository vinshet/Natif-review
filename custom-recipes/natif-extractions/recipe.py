...
def get_category_values(val,sub_cat=''):
    if val is None and sub_cat!='bbox_refs':
        append_to_list(category_name,sub_cat,'None',upload_type,doc_name,'None','None')
    elif isinstance(val,dict):
        if 'value' in val:
            b_id,p_num=get_box_details(val['bbox_refs'])
            append_to_list(category_name,sub_cat,val['value'],upload_type,doc_name,b_id,p_num)
        else:
        ...
        if val:
            append_to_list(category_name,sub_cat,val,upload_type,doc_name,'None','None')
...
def write_to_dataset(file_info):
    ...
    if check_response(upload_details):
        ...
        if upload_details.json()["processing_status"] == "pending":
            ...
            write_to_log("Error retreiving extractions from document {}".format(file_name))
...
def create_default_files(doc_type):
    with input_handle.get_writer(str("/" + doc_type.title() + "/"+doc_type.lower()+"_log.txt")) as w:
    ...
    if check_structure():
        ...
        for file_path in input_folder_paths:
            ...
            if file_type not in allowed_filetypes:
                write_to_log("File type not allowed for document {}".format(os.path.basename(file_path)))
                continue
        ...
        raise ValueError(
        "Documents are not placed within their appropriate folders"
    )
    rename_duplicates()
    extractions_dataset={'Document_name':document_name,'Document_type':doc,'category':l1,'sub-category':l2,'text':l3,'page_number':page_number,"box_id":box_id}
    df=pd.DataFrame(extractions_dataset,columns=['Document_name','Document_type','category','sub-category','text','page_number','box_id'])
    df=df.applymap(str)
    if len(df)>0:
